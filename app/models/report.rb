class Report < ActiveRecord::Base
	...
	def self.get_processes(filter_column=nil, filter=nil)
		filter_column ||= nil
		filter ||= nil
		...
		if filter_column && filter # only filter if a value is provided
			...
			if %w(pid pgid user).include? filter_column
				process_hash.each do |item|
					...
					if (item[:process_id] == filter || 
						item[:process_group_id] == filter ||
						item[:process_user] == filter) # add to new hash if exact match of values
						filtered_hash << item
					end
				end
